...
\section{Experiments}
 ...
 \item Easy to Hard Model - Trained with boostrapped active curriculum construct from the above baseline model, beginning with the easiest/least uncertain task and including the other tasks throughout training.
...
All models are tested on the same test set of 5000 images, all containing `hard' geometric shapes; experiments are repeated multiple times with different weight initialisations. To analyse the effect of the curriculum on learning we report the accuracy of the different models over the held out test sets. The exact architectures model architectures are given in tables 5.1 - 5.3 below (`FC' = fully connected Layer):
